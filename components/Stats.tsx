...
export const Stats: React.FC<StatsProps> = ({ data, onCardClick }) => {
  ...
  const cardClass = "bg-white p-6 rounded-xl shadow-sm border border-slate-100 flex items-center justify-between cursor-pointer hover:shadow-md hover:border-blue-200 transition-all group relative overflow-hidden";
  const clickHint = "absolute right-2 top-2 opacity-0 group-hover:opacity-100 transition-opacity text-slate-300";

  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
      
      {/* Monto Total Card */}
      <div className={cardClass} onClick={() => onCardClick?.('total')}>
        <div className={clickHint}><ChevronRight size={16}/></div>
        <div>
          <p className="text-sm text-slate-500 font-medium">Monto Total</p>
          <p className="text-2xl font-bold text-slate-900">{formatCurrency(totalValue)}</p>
          <p className="text-xs text-slate-400 mt-1">Click para detalle</p>
        </div>
      ...
      <div className={cardClass} onClick={() => onCardClick?.('trucks')}>
        <div className={clickHint}><ChevronRight size={16}/></div>
        <div>
          ...
          <p className="text-xs text-slate-400 mt-1">Ver patentes únicas</p>
        </div>
      ...
      {/* Operaciones Card */}
      <div className={cardClass} onClick={() => onCardClick?.('ops')}>
        <div className={clickHint}><ChevronRight size={16}/></div>
        <div>
