...
import { getFiles, deleteFile } from '../utils/storage';
...
export const ReportsView: React.FC<ReportsViewProps> = ({ data }) => {
    ...
    const handleDeleteFile = async (id: string) => {
        if (confirm('¿Estás seguro de eliminar este archivo del historial?')) {
            await deleteFile(id);
            loadFiles();
        }
    };
    ...
    const exportFullReport = () => {
        ...
        XLSX.writeFile(wb, `LogisticaAI_Completo_${new Date().toISOString().slice(0,10)}.xlsx`);
    };
    ...
    return (
        <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
            ...
            {/* Acciones Rápidas (Botones Didácticos) */}
