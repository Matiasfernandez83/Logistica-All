...
import { Database, CheckCircle2, ArrowRightLeft, FileSpreadsheet, AlertTriangle, RefreshCw, Menu } from 'lucide-react';
...
import { getRecords, saveRecords, getFleet, saveFleet, saveFiles, clearRecords, getTheme, saveTheme, getFileById, getExpenses, deleteRecords } from './utils/storage';
      ...
      <main className="md:ml-64 p-4 md:p-8 flex-1 overflow-y-auto h-screen w-full">
        <header className="flex flex-col md:flex-row justify-between md:items-center mb-8 gap-4">
          <div className="flex items-center gap-4">
             ...
             <div>
                <h2 className="text-xl md:text-2xl font-bold text-slate-800">
                    ...
                    currentView === 'import' ? 'Importador' : 
                    currentView === 'expenses' ? 'Gastos Tarjetas' :
                ...
                {(() => {
                    ...
                    try {
                        ...
                        if (!hasKey && typeof window !== 'undefined' && (window as any).process?.env?.API_KEY) hasKey = true;
                    } catch(e) {}
                    
                    if (!hasKey) {
                        return (
                            <div className="hidden md:flex items-center gap-2 mt-2 text-red-600 bg-red-50 px-3 py-1 rounded text-sm font-bold border border-red-200">
                                <AlertTriangle size={16} />
                                <span>ADVERTENCIA: API_KEY no detectada.</span>
                            </div>
                        );
                    }
                    return null;
          ...
          <button 
            ...
            className={`w-full md:w-auto flex items-center justify-center gap-2 px-4 py-2 bg-white border border-slate-200 rounded-lg text-sm font-medium text-slate-600 hover:text-blue-600 hover:border-blue-200 transition-all shadow-sm ${isRefreshing ? 'opacity-50' : ''}`}
          >
        ...
        {currentView === 'settings' ? (
              ...
              <ExpensesView 
                  ...
                  onViewDetail={(title, records) => setModalData({ isOpen: true, title, type: 'list', records })}
                  theme={theme}
              />
        ) : currentView === 'import' ? (
                  ...
                  <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500 pb-12">
                      ...
                      {records.length > 0 ? (
                        <>
                            <Stats data={records} onCardDoubleClick={handleStatClick} />
                            <Charts data={records} onBarClick={handleChartClick} />
                            <DataTable 
                                data={records} 
